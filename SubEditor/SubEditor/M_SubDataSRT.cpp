...
int M_SubDataSRT::size(){
	return counter;
}
bool M_SubDataSRT::empty(){
	return counter == 0;
}
int M_SubDataSRT::getEncoding(){
	return encoding;
}
String^ M_SubDataSRT::getPath(){
	return path;
}
String^ M_SubDataSRT::getName(){
	return name;
}
void M_SubDataSRT::setEncoding(int enc){
	encoding = enc;
}
void M_SubDataSRT::setPath(String^ path){
	this->path = path;
}
void M_SubDataSRT::setName(String^ name){
	this->name = name;
}
void M_SubDataSRT::insert(M_SubData::nodeData^ data){
	moveEnd();
...
void M_SubDataSRT::insertIn(int pos, M_SubData::nodeData^ data){
	if(currentNode != nullptr){
		node^ n = gcnew node;
		n->data = data;
		moveTo(pos);
...
void M_SubDataSRT::erase(int pos){
	moveTo(pos);
	if(pos > 1){
...
void M_SubDataSRT::edit(int pos){
}
void M_SubDataSRT::clear(){
...
void M_SubDataSRT::increaseIndex(){
	node^ n = currentNode;
...
void M_SubDataSRT::decreseIndex(){
	node^ n = currentNode;
...
void M_SubDataSRT::moveStart(){
	currentNode = firstNode;	
}
void M_SubDataSRT::moveEnd(){
	currentNode = lastNode;
}
void M_SubDataSRT:: movePrev(){
	if(currentNode == firstNode) return;
	node^ aux = currentNode;
	currentNode = aux->prev;
}
void M_SubDataSRT::moveNext(){
	if(currentNode == lastNode) return;
	node^ aux = currentNode;
	currentNode = aux->next;
}
void M_SubDataSRT::moveTo(int index){
